...
export function OperationalTab({ view, selection: initial }: {
    ...
    {view === "capacity" ? <><FilteredTrend title="领件趋势" rows={rows} series={capacityColumns.slice(0, 5).map((m) => ({ metric: m, type: m.unit === "%" ? "line" : "bar" }))} targets={[capacityMetrics[0]]} selection={selection}/><Analysis selection={selection} title="派送构成趋势" option={comboOption(rows, [metric("delivered", "妥投量"), metric("exception", "派送异常量")].map((m) => ({ metric: m, type: "bar", stack: "total" })), undefined, Boolean(selection.range), !selection.range)} height="primary"/><Distribution records={records} selection={selection}/><MetricTable metrics={capacityColumns} rows={rows} summary={summary} selection={selection}/></> : null}
    ...
    {view === "quality" ? <><Choices value={anchor} onChange={(v) => { setAnchor(v); document.getElementById(`quality-section-${v}`)?.scrollIntoView({ behavior: "smooth", block: "start" }); }} options={qualityGroups.map((g, i) => [String(i), g.title])} label="质量分析章节"/>{qualityGroups.map((g, i) => <section key={g.title} id={`quality-section-${i}`} className="flex scroll-mt-20 flex-col gap-4"><h2 className="text-lg font-medium">{g.title}</h2><Kpis metrics={g.metrics} values={values} selection={selection} onDetail={setDetail}/>{i < 2 ? <FilteredTrend title={`${g.title}趋势`} rows={rows} series={(i === 0 ? [g.metrics[3], g.metrics[0], g.metrics[1]] : g.metrics).map((m) => ({ metric: m, type: m.unit === "%" ? "line" : "bar" }))} targets={g.metrics.filter((m) => m.target !== undefined)} selection={selection}/> : <><Analysis selection={selection} title="客诉趋势" option={comboOption(rows, g.metrics.map((m) => ({ metric: m, type: m.unit === "%" ? "line" : "bar" })), undefined, Boolean(selection.range), !selection.range)} height="primary"/><Distribution records={records} selection={selection} complaint/></>}</section>)}<MetricTable metrics={qualityColumns} rows={rows} summary={summary} selection={selection} quality/></> : null}

...
export function donutOption(items: {
    ...
    return { aria: { enabled: true }, textStyle: { fontSize: 12 }, tooltip: { trigger: "item", confine: true, formatter: (p) => { const item = (Array.isArray(p) ? p[0] : p) as {
                ...
                rows: [{
                    marker: item.marker,
                    label: "数量 / 占比",
                    value: `${display(item.value, unit)} · ${Number.isFinite(item.percent) ? `${item.percent}%` : "—"}`,
                }],
            }); } }, legend: chartLegend("bottom", true), title: { show: selected === undefined, text: display(total, unit === "min" ? "min" : ""), subtext: unit === "min" ? "工作时长" : `总${unit === "人" ? "人数" : "量"}`, left: "center", top: "38%", textStyle: { fontSize: 20 }, subtextStyle: { fontSize: 12 } }, series: [{ type: "pie", radius: ["48%", "68%"], center: ["50%", "45%"], data: items.map((item) => ({ ...item, selected: item.name === selected })), label: { show: false, fontSize: 12 }, selectedMode: "single", emphasis: { scale: true } }] };
}
function Distribution({ records, selection, organizationId, complaint = false }: {
    records: Daily[];
    selection: Selection;
    complaint?: boolean;
    ...
    const values = aggregate(records), allChartRows = byPerspective(records, perspective);
    const m = metric(category === "all" ? totalKey : key(names.indexOf(category)), category === "all" ? "全部" : category);
    const chartRows = perspective === "driver" && allChartRows.length > 20
        ? [...allChartRows].sort((a, b) => (b.values[m.key] || 0) - (a.values[m.key] || 0)).slice(0, 20)
        : allChartRows;
    ...
    const rows: RecordRow[] = byPerspective(records, listPerspective).map((r) => ({ ...r, children: selectedPeriods(selection).map((period) => {
            const current = byPerspective(recordsFor({ mode: selection.mode, value: period }, organizationId), listPerspective).find((x) => x.id === r.id);
...
export function OperationalTab({ view, selection: initial, organizationId }: {
    ...
    const records = recordsFor(selection, organizationId), values = aggregate(records), rows = periodRows(selection, organizationId);
    const summary: RecordRow = { id: "summary", label: "总汇总", period: selection.range?.end || selection.value, values: aggregate(recordsFor(selection, organizationId, true)) };
    ...
    {view === "capacity" ? <><FilteredTrend title="领件趋势" rows={rows} series={capacityColumns.slice(0, 5).map((m) => ({ metric: m, type: m.unit === "%" ? "line" : "bar" }))} targets={[capacityMetrics[0]]} selection={selection}/><Analysis selection={selection} title="派送构成趋势" option={comboOption(rows, [metric("delivered", "妥投量"), metric("exception", "派送异常量")].map((m) => ({ metric: m, type: "bar", stack: "total" })), undefined, Boolean(selection.range), !selection.range)} height="primary"/><Distribution records={records} selection={selection} organizationId={organizationId}/><MetricTable metrics={capacityColumns} rows={rows} summary={summary} selection={selection}/></> : null}
    {view === "efficiency" ? <><Analysis selection={selection} title="PPH趋势" option={comboOption(rows, [{ metric: efficiencyMetrics[0], type: "bar" }, { metric: { ...efficiencyMetrics[1], unit: "件/小时" } }], undefined, Boolean(selection.range), !selection.range)} height="primary"/><div className="grid gap-4 xl:grid-cols-2"><Analysis selection={selection} title="工时趋势" action={<Choices value={trendHours} onChange={setTrendHours} options={[["total", "总时长"], ["average", "人均时长"]]} label="工时趋势口径"/>} option={comboOption(rows, efficiencyColumns.slice(2, 5).map((m) => ({ metric: { ...m, key: trendHours === "total" ? m.key : `avg${m.key}` } })), undefined, Boolean(selection.range), !selection.range)}/><Analysis selection={selection} title="工时结构" description={`派件司机 ${values.drivers} 人；工作时长为分拣、首单、派件时长合计。`} action={<Choices value={pieHours} onChange={setPieHours} options={[["total", "总时长"], ["average", "人均时长"]]} label="工时结构口径"/>} option={donutOption(efficiencyColumns.slice(2, 5).map((m) => ({ name: m.label, value: values[pieHours === "total" ? m.key : `avg${m.key}`] })), "min")}/></div><MetricTable metrics={efficiencyColumns} rows={rows} summary={summary} selection={selection}/></> : null}
    {view === "timeliness" ? <><FilteredTrend title="时效趋势" rows={rows} series={[{ metric: timelinessMetrics[0] }, { metric: metric("raw2400", "2400妥投率（原始值）", "%"), hidden: true }, { metric: timelinessMetrics[1] }, { metric: metric("raw4800", "4800妥投率（原始值）", "%"), hidden: true }, { metric: timelinessMetrics[2] }]} targets={timelinessMetrics.slice(0, 3)} selection={selection}/><Tabs defaultValue="data"><TabsList variant="line"><TabsTrigger value="data">数据明细</TabsTrigger><TabsTrigger value="reason">剔除原因汇总</TabsTrigger><TabsTrigger value="waybill">剔除运单明细</TabsTrigger></TabsList><TabsContent value="data" className="flex flex-col gap-4"><Choices value={raw} onChange={setRaw} options={[["off", "仅考核值"], ["on", "对比原始值"]]} label="数据明细口径"/><MetricTable metrics={timelinessColumns} rows={rows} summary={summary} selection={selection} rawCompare={raw === "on"}/></TabsContent><ExclusionTables selection={selection} organizationId={organizationId} embedded/></Tabs></> : null}
    {view === "quality" ? <><Choices value={anchor} onChange={(v) => { setAnchor(v); document.getElementById(`quality-section-${v}`)?.scrollIntoView({ behavior: "smooth", block: "start" }); }} options={qualityGroups.map((g, i) => [String(i), g.title])} label="质量分析章节"/>{qualityGroups.map((g, i) => <section key={g.title} id={`quality-section-${i}`} className="flex scroll-mt-20 flex-col gap-4"><h2 className="text-lg font-medium">{g.title}</h2><Kpis metrics={g.metrics} values={values} selection={selection} onDetail={setDetail}/>{i < 2 ? <FilteredTrend title={`${g.title}趋势`} rows={rows} series={(i === 0 ? [g.metrics[3], g.metrics[0], g.metrics[1]] : g.metrics).map((m) => ({ metric: m, type: m.unit === "%" ? "line" : "bar" }))} targets={g.metrics.filter((m) => m.target !== undefined)} selection={selection}/> : <><Analysis selection={selection} title="客诉趋势" option={comboOption(rows, g.metrics.map((m) => ({ metric: m, type: m.unit === "%" ? "line" : "bar" })), undefined, Boolean(selection.range), !selection.range)} height="primary"/><Distribution records={records} selection={selection} organizationId={organizationId} complaint/></>}</section>)}<MetricTable metrics={qualityColumns} rows={rows} summary={summary} selection={selection} quality/></> : null}
    {detail ? <MetricDetail key={detail.key} type={detail} selection={selection} organizationId={organizationId} onClose={() => setDetail(undefined)}/> : null}{assessment ? <AssessmentDetail type={assessment} selection={selection} organizationId={organizationId} onClose={() => setAssessment(undefined)}/> : null}
  </div>;
}
export function OverviewOperations({ organizationId, onNavigate }: {
    organizationId: string;
    ...
    const values = aggregate(recordsFor(selection, organizationId)), previous = aggregate(recordsFor({ ...selection, value: shiftPeriod(selection.mode, selection.value, -1) }, organizationId)), week = selection.mode === "day" ? aggregate(recordsFor({ mode: "day", value: shiftPeriod("day", selection.value, -7) }, organizationId)) : undefined;
    const sections: {
        title: string;
        view: CockpitView;
        metrics: Metric[];
    ...
    return <><Card><CardHeader><CardTitle>运营指标</CardTitle><CardDescription>{MOCK_NOTICE}</CardDescription></CardHeader><CardContent className="flex flex-col gap-4"><CockpitTimeFilter selection={selection} onChange={setSelection} allowRange={false}/><Choices value={filter} onChange={setFilter} options={[["all", "全部指标"], ["assessment", "仅考核指标"]]} label="指标筛选"/>{sections.filter((s) => filter === "all" || s.metrics.some((m) => m.assessment)).map((s) => <section key={s.view} className="flex flex-col gap-4 border-t pt-4"><div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-medium">{s.title}</h3>{s.view === "timeliness" ? <div className="flex items-center gap-2">{filter === "all" ? <Choices value={scope} onChange={setScope} options={[["assessment", "考核口径"], ["raw", "原始口径"]]} label="时效口径"/> : <span className="text-xs text-muted-foreground">考核口径</span>}<MetricHelp /></div> : null}<Button variant="link" size="sm" onClick={() => onNavigate(s.view, selection)}>详情<ArrowRightIcon data-icon="inline-end"/></Button></div><Kpis metrics={s.metrics.filter((m) => filter === "all" || m.assessment)} values={values} selection={selection} onDetail={setDetail} comparison={{ previous, week }} raw={s.view === "timeliness" && filter !== "assessment" && scope === "raw"}/></section>)}</CardContent></Card><Card><CardHeader><CardTitle>司机表现</CardTitle><CardDescription>月度数据 · 每月5号计划更新 · {periodLabel("month", month)}</CardDescription><Button variant="link" size="sm" onClick={() => onNavigate("driver", { mode: "month", value: month })}>详情<ArrowRightIcon data-icon="inline-end"/></Button></CardHeader><CardContent className="flex flex-col gap-4">{month !== selection.value.slice(0, 7) ? <Alert><AlertDescription>所选周期所属月份尚未就绪，展示最近已就绪月份。</AlertDescription></Alert> : null}<Kpis metrics={driverSummaryMetrics} values={driverSummary(driverMonth(month, organizationId))} selection={{ mode: "month", value: month }} comparison={{ previous: driverSummary(driverMonth(shiftPeriod("month", month, -1), organizationId)) }}/></CardContent></Card>{detail ? <MetricDetail key={detail.key} type={detail} selection={selection} organizationId={organizationId} onClose={() => setDetail(undefined)}/> : null}</>;
}

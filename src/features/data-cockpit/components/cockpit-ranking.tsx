...
export function CockpitRanking() {
    const { selection, choose, notice, dismiss } = useReadySelection("month", "week");
    const [scope, setScope] = useState("station");
    const mode = selection.mode as RankMode, rows = rankingRows(mode, selection.value), current = rows[0], previous = rows[1], available = rankAvailable(mode, current.id), previousAvailable = rankAvailable(mode, previous.id);
    ...
    trend.yAxis = { type: "value", inverse: true, min: 1, minInterval: 1, name: "名次", axisLabel: { fontSize: 12 } };
    ...
    const radar: EChartsOption = { aria: { enabled: true }, tooltip: { trigger: "item", confine: true, formatter: (p) => { const item = (Array.isArray(p) ? p[0] : p) as {
                name?: string;
            }; const values = item.name === "当前DSP" ? dimensions.map((d) => current.values[d.key]) : leader; return `<div class="cockpit-chart-tooltip"><strong>${item.name}</strong>${dimensions.map((d, i) => `<div><span>${d.label}（${d.parts}）</span><span>${display(values[i], "分")} / ${d.max}</span></div>`).join("")}</div>`; } }, legend: { bottom: 0, textStyle: { fontSize: 12 } }, radar: { radius: "65%", indicator: dimensions.map((d) => ({ name: d.label, max: 100 })), axisName: { fontSize: 12 } }, series: [{ type: "radar", data: [{ name: "当前DSP", value: dimensions.map((d) => current.values[d.key] / d.max * 100) }, ...(available ? [{ name: scope === "station" ? "站点第一名" : "大区第一名", value: leader.map((v, i) => v / dimensions[i].max * 100) }] : [])] }] };

...
function DimensionDetail({ dimension, rows, month, onClose }: {
    ...
    return <DetailShell title={`${dimension.label}详情`} description={`${periodLabel("month", month)} · 满分${dimension.max}分 · ${MOCK_NOTICE}`} onClose={onClose}><Tabs defaultValue="score"><TabsList variant="line"><TabsTrigger value="score">分数趋势</TabsTrigger><TabsTrigger value="details">详情趋势</TabsTrigger></TabsList><TabsContent value="score"><Analysis selection={selection} title={`${dimension.label} · 司机分数对比`} description="此图比较所选月的司机得分，不是跨月趋势。" option={scores} height="detail"/></TabsContent><TabsContent value="details"><Analysis selection={selection} title={`${dimension.label} · 原始指标对比`} option={comboOption(sorted, dimension.metrics.map((m) => ({ metric: m, type: dimension.key === "efficiency" ? "bar" : "line" })))} height="detail"/></TabsContent></Tabs><DataGrid title="维度详情" columns={scoreColumns([metric("name", "司机"), metric(dimension.key, dimension.label, "分"), metric(`${dimension.key}Level`, "等级"), ...dimension.metrics])} rows={sorted} rowKey={(r) => r.id} filename={`${dimension.label}_${month}`}/></DetailShell>;
}

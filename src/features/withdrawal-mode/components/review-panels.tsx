...
export function AuditPanel({ row, stage, onSubmit, ...props }: PanelBaseProps & { stage: "business" | "financial"; onSubmit: (result: "pass" | "reject", reason: string) => void }) {
  ...
  return <>
    <WorkflowPanel {...props} title={stage === "business" ? "业务审核" : "财务审核"} description={`${row.dspName} · ${row.fleetName}`} dirty={dirty} footer={(requestClose) => <><Button variant="outline" onClick={requestClose}>取消</Button><Button onClick={submit}>提交审核</Button></>}>

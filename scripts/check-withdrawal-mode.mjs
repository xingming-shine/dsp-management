...
test("closing with drivers records next midnight and reopening preserves history", () => {
  ...
  assert.equal(closed[0].modeStatus, "closing_pending_effective")
  assert.throws(() => transition(closed, { type: "open", id: "test", existingId: "test", draft }, now, "Tester"), /不能重新提交/)
  const reopened = transition([{ ...closed[0], modeStatus: "closed" }], { type: "open", id: "test", existingId: "test", draft }, now, "Tester")

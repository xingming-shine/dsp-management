...
- Period picker: use the project `PeriodPicker` for day, week, and month values and ranges. Only its leading day/week/month granularity selector uses the 36px `raised` Tabs variant; the single/range switch inside the panel remains `line`, and unrelated Tabs are unchanged. Its trigger is 360px maximum width and 36px high; its borderless shadowed Popover is 460px wide for single day/week/month selection and 920px wide for ranges, with both widths capped to the viewport. Calendar padding and the gap between paired calendars are 20px. Cells are based on 36px, range fill uses 8% brand orange in light mode, and range shortcuts are 7/14/31 days, 4/8/12 weeks, or 3/6/12 months. The footer is at least 56px high with 20px horizontal and 10px vertical padding; summary stays left while 36px-high Cancel and Apply Buttons stay right. Future periods are disabled, and all display/storage values follow the shared US-order and ISO rules.
